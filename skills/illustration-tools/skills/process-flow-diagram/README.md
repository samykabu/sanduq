...
Turn a plain-English description of a workflow into a polished, dark-themed **process flow diagram** rendered as a single self-contained HTML file (inline SVG + embedded CSS). Best for sequential, step-by-step processes: approval flows, automation pipelines, runbooks, onboarding, and decision trees.
...
3. **Claude copies and customizes the template** — it starts from [`resources/template.html`](resources/template.html), lays out numbered step boxes left-to-right, adds decision diamonds, draws labeled arrows, and fills the summary cards.
...
- **Semantic color palette** — each step type has a fixed fill + stroke:

  | Step type | Color | Fill (rgba) | Stroke | Shape/Indicator |
  |-----------|-------|-------------|--------|-----------------|
  | Start/End | Cyan | `rgba(8, 51, 68, 0.4)` | `#22d3ee` | Pill shape |
  | Manual | Emerald | `rgba(6, 78, 59, 0.4)` | `#34d399` | 👤 actor |
  | Automated | Violet | `rgba(76, 29, 149, 0.4)` | `#a78bfa` | ⚡ / 🤖 |
  | Integration/API | Amber | `rgba(120, 53, 15, 0.3)` | `#fbbf24` | 🔗 / ☁️ |
...
- **Canvas** — `#020617` (slate-950) background with a 40px SVG grid.
...
Identical to the architecture skill: a collapsible `⋯` header toolbar with **📋 Copy / 🖼️ PNG / 📄 PDF**, all using one [html2canvas](https://html2canvas.hertzen.com/) capture (toolbar excluded, 32px padding) so PDF keeps the dark theme. Two CDN scripts, both **pinned with SRI hashes** and `crossorigin="anonymous"`:
...
Edit the **template** to change the default for all future diagrams, or a **generated file** to tweak one. Just ask Claude, or edit by hand:
...
<text x="CENTER_X" y="Y+48" fill="#94a3b8" font-size="9" text-anchor="middle">Description</text>
```
