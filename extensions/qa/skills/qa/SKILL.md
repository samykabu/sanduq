---
name: qa
description: Analyze test and documentation readiness for an active Spec Kit feature, generate or refresh a feature-scoped QA test manual, and enforce lifecycle freshness before implementation or pull-request creation. Use for QA initialization, QA.Analyze, QA.Document, test scenarios, screenshot evidence, API samples, accessibility checks, or QA lifecycle gates.
---
...
- For initialization, configure `qa-config.yml` and lifecycle hooks.
...
7. Use `scripts/qa_state.py` to determine freshness; do not treat a file's existence as proof that it
   is current.
